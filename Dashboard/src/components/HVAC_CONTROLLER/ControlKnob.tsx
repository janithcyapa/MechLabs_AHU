export const ControlKnob = ({ label, value, onChange, icon: Icon, colorClass = "text-cyan-400", isBinary = false, disabled = false }: any) => {
  ...
  const disabledClasses = disabled ? "opacity-30 pointer-events-none grayscale" : "";

  return (
    <div className={`flex flex-col items-center justify-between gap-3 bg-[#111827] p-4 rounded-xl border border-slate-700/50 shadow-inner min-w-30 h-full ${disabledClasses}`}>
...
export const ToggleSwitch = ({
  ...
  return (
    <div className={`flex flex-col gap-3 bg-[#111827] p-4 rounded-xl border border-slate-700/50 shadow-inner ${disabledClasses}`}>
      ...
      {/* The Toggle Switch Container (Low Height) */}
      <div className="relative flex items-center justify-center h-6 mt-1">
        ...
        <button
          ...
          className="absolute w-14 h-6 z-10 cursor-pointer outline-none rounded-full"
          title={isOn ? "Turn OFF" : "Turn ON"}
        />

        {/* The Track */}
        <div className={`w-14 h-6 rounded-full shadow-[inset_0_2px_8px_rgba(0,0,0,0.8),0_2px_4px_rgba(255,255,255,0.05)] border transition-colors duration-300 flex items-center px-1 ${isOn ? 'border-slate-600 bg-slate-800' : 'border-slate-800 bg-[#0f172a]'
          }`}>

          {/* The Sliding Thumb */}
          <div
            className={`w-4 h-4 rounded-full bg-slate-700 shadow-[0_2px_6px_rgba(0,0,0,0.6),inset_0_2px_2px_rgba(255,255,255,0.2)] border border-slate-600 flex items-center justify-center transition-transform duration-300 ease-out ${isOn ? `translate-x-8 ${colorClass}` : 'translate-x-0 text-slate-500'
              }`}
          >
            {/* The Glowing Indicator Mark */}
            <div className={`w-1 h-1 rounded-full transition-all duration-300 ${isOn ? 'bg-current shadow-[0_0_6px_currentColor]' : 'bg-slate-500'
              }`}></div>
          </div>
        </div>
      </div>
