...
PwmUtil led_red(27, LEDC_CHANNEL_2, LEDC_TIMER_1);
PwmUtil led_green(26, LEDC_CHANNEL_3, LEDC_TIMER_1);
PwmUtil led_blue(25, LEDC_CHANNEL_4, LEDC_TIMER_1);
...
// Separate instances so internal calibration/states don't collide across channels
...
// ==========================================
void init_relays() {
    const gpio_num_t relays[] = {(gpio_num_t)PIN_RELAY_COOL, (gpio_num_t)PIN_RELAY_HEAT, (gpio_num_t)PIN_RELAY_HUM, (gpio_num_t)PIN_RELAY_EXTRA};
    for (int i = 0; i < 4; i++) {
...
void read_publish_aht_ens(uint8_t ch, sens_aht21::AHT21 &aht, sens_ens160::ENS160 &ens, const char* topic_suffix, uint32_t ts, bool &err_flag) {
    float t = 0.0f, h = 0.0f; 
    uint16_t co2 = 0;
    ...
    if (select_mux_channel(ch) == ESP_OK) {
        if (aht.read(t, h) == ESP_OK) {
            ...
            if (ens.read_data(ens_data) == ESP_OK) co2 = ens_data.eco2;
        } else {
            err_flag = true;
        }
    } else {
...
void on_ws_cmd(const char *data) {
    ...
    if ((item = cJSON_GetObjectItem(root, "cool_coil"))) {
        ...
        gpio_set_level((gpio_num_t)PIN_RELAY_COOL, actuators.cool_coil > 0 ? RELAY_ON_STATE : RELAY_OFF_STATE);
    }
    if ((item = cJSON_GetObjectItem(root, "heat_coil"))) {
        actuators.heat_coil = (float)item->valuedouble;
        gpio_set_level((gpio_num_t)PIN_RELAY_HEAT, actuators.heat_coil > 0 ? RELAY_ON_STATE : RELAY_OFF_STATE);
    }
...
extern "C" void app_main(void) {
    ...
    led_red.init(5000); led_green.init(5000); led_blue.init(5000);
    buzzer.init(2000);
    ...
    while (1) {
        ...
        bool channel_error = false;    // Temporary flag for the current sensor being read
        ...
        channel_error = false;
        read_publish_aht_ens(7, aht_return, ens_return, "return", uptime_ms, channel_error);
        if (channel_error) { ESP_LOGW(TAG, "Fault detected on 'return' sensor (CH 7)"); any_sensor_error = true; }

        channel_error = false;
        read_publish_aht_ens(6, aht_mixed, ens_mixed, "mix", uptime_ms, channel_error);
        if (channel_error) { ESP_LOGW(TAG, "Fault detected on 'mix' sensor (CH 6)"); any_sensor_error = true; }

        channel_error = false;
        read_publish_bme(5, bme_cooler, "cooler", uptime_ms, channel_error);
        if (channel_error) { ESP_LOGW(TAG, "Fault detected on 'cooler' sensor (CH 5)"); any_sensor_error = true; }

        channel_error = false;
        read_publish_bme(4, bme_heater, "heater", uptime_ms, channel_error);
        if (channel_error) { ESP_LOGW(TAG, "Fault detected on 'heater' sensor (CH 4)"); any_sensor_error = true; }

        channel_error = false;
        read_publish_aht_ens(3, aht_release, ens_release, "release", uptime_ms, channel_error);
        if (channel_error) { ESP_LOGW(TAG, "Fault detected on 'release' sensor (CH 3)"); any_sensor_error = true; }

        channel_error = false;
        read_publish_aht_ens(2, aht_outdoor, ens_outdoor, "outside", uptime_ms, channel_error);
        if (channel_error) { ESP_LOGW(TAG, "Fault detected on 'outside' sensor (CH 2)"); any_sensor_error = true; }
